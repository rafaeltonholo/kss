...
class CssTokenizerTest {
    ...
    private fun `when url is incomplete - then should create a BadUrl token`(url: BadUrlTokenParam) {
        println("url: '$url'")
        // Arrange (cont.)
        val content =
            """
    ...
    @Test
    fun `url followed by quote creates Function token - double quotes empty url`() =
        `url followed by quote should create Function token`(
            UrlAsFunctionParam.`with a double quotes empty url`
        )

    @Test
    fun `url followed by quote creates Function token - double quotes hashed url`() =
        `url followed by quote should create Function token`(
            UrlAsFunctionParam.`with a double quotes hashed url`
        )

    @Test
    fun `url followed by quote creates Function token - double quotes url`() =
        `url followed by quote should create Function token`(
            UrlAsFunctionParam.`with a double quotes url`
        )

    @Test
    fun `url followed by quote creates Function token - double quotes leading spaced url`() =
        `url followed by quote should create Function token`(
            UrlAsFunctionParam.`with a double quotes leading spaced url`
        )

    @Test
    fun `url followed by quote creates Function token - double quotes trailing spaced url`() =
        `url followed by quote should create Function token`(
            UrlAsFunctionParam.`with a double quotes trailing spaced url`
        )

    @Test
    fun `url followed by quote creates Function token - double quotes spaced url`() =
        `url followed by quote should create Function token`(
            UrlAsFunctionParam.`with a double quotes spaced url`
        )

    @Test
    fun `url followed by quote creates Function token - single quotes empty url`() =
        `url followed by quote should create Function token`(
            UrlAsFunctionParam.`with a single quotes empty url`
        )

    @Test
    fun `url followed by quote creates Function token - single quotes hashed url`() =
        `url followed by quote should create Function token`(
            UrlAsFunctionParam.`with a single quotes hashed url`
        )

    @Test
    fun `url followed by quote creates Function token - single quotes url`() =
        `url followed by quote should create Function token`(
            UrlAsFunctionParam.`with a single quotes url`
        )

    @Test
    fun `url followed by quote creates Function token - single quotes leading spaced url`() =
        `url followed by quote should create Function token`(
            UrlAsFunctionParam.`with a single quotes leading spaced url`
        )

    @Test
    fun `url followed by quote creates Function token - single quotes trailing spaced url`() =
        `url followed by quote should create Function token`(
            UrlAsFunctionParam.`with a single quotes trailing spaced url`
        )

    @Test
    fun `url followed by quote creates Function token - single quotes spaced url`() =
        `url followed by quote should create Function token`(
            UrlAsFunctionParam.`with a single quotes spaced url`
        )

    private fun `url followed by quote should create Function token`(url: UrlAsFunctionParam) {
        println("url: '$url'")
        // Arrange (cont.)
        val content =
            """
        ...
        val leadingWhitespaceEndOffset =
            if (url.startsWith(' ')) {
                openParenthesisEndOffset + url.indexOfFirst { it == '"' || it == '\'' } - 1
            } else {
                -1
            }
        val trailingWhitespaceEndOffset =
            if (url.endsWith(' ')) {
    ...
    fun `create tokens for css property for numbers - with a dimension starting with a plus`() =
        `create tokens for css property for numbers`(
            NumberOrDimensionTokenParam.`with a dimension starting with a plus`
        )

    @Test
    fun `create tokens for css property for numbers - with a dimension starting with a minus`() =
        `create tokens for css property for numbers`(
            NumberOrDimensionTokenParam.`with a dimension starting with a minus`
        )

    @Test
    fun `create tokens for css property for numbers - with a dimension starting with a dot and a plus`() =
        `create tokens for css property for numbers`(
            NumberOrDimensionTokenParam.`with a dimension starting with a dot and a plus`
        )

    @Test
    fun `create tokens for css property for numbers - with a dimension starting with a dot and a minus`() =
        `create tokens for css property for numbers`(
            NumberOrDimensionTokenParam.`with a dimension starting with a dot and a minus`
        )

    @Test
    fun `create tokens for css property for numbers - with a dimension with scientific notation`() =
        `create tokens for css property for numbers`(
            NumberOrDimensionTokenParam.`with a dimension with scientific notation`
        )

    @Test
    fun `create tokens for css property for numbers - with a dimension with scientific notation and a plus`() =
        `create tokens for css property for numbers`(
            NumberOrDimensionTokenParam.`with a dimension with scientific notation and a plus`
        )

    @Test
    fun `create tokens for numbers - dimension with scientific notation starting with plus`() =
        `create tokens for css property for numbers`(
            NumberOrDimensionTokenParam.`with a dimension with scientific notation starting with a plus`
        )

    @Test
    fun `create tokens for numbers - dimension with scientific notation starting with minus`() =
        `create tokens for css property for numbers`(
            NumberOrDimensionTokenParam.`with a dimension with scientific notation starting with a minus`
        )

    @Test
    fun `create tokens for css property for numbers - with a dimension with scientific notation starting with a dot`() =
        `create tokens for css property for numbers`(
            NumberOrDimensionTokenParam.`with a dimension with scientific notation starting with a dot`
        )
    ...
    fun `create tokens for a rule with BadUrl - with incomplete double-quoted string with escaped quote`() =
        `create tokens for a rule with BadUrl`(
            BadStringParams.`with incomplete double-quoted string with escaped quote`
        )
    ...
    fun `create tokens for a rule with BadUrl - with incomplete single-quoted string with escaped quote`() =
        `create tokens for a rule with BadUrl`(
            BadStringParams.`with incomplete single-quoted string with escaped quote`
        )
    ...
    fun `given a minified style - when creating tokens - then should create a valid token list - no spaces`() =
        `given a minified style - when creating tokens - then should create a valid token list`(
            MinifiedCssParams.`no spaces`
        )

    @Test
    fun `minified style creates valid token list - no spaces and dimensions starting with dot`() =
        `given a minified style - when creating tokens - then should create a valid token list`(
            MinifiedCssParams.`no spaces and dimensions starting with dot`
        )
    ...
    @Suppress("unused", "EnumEntryName", "ktlint:standard:enum-entry-name-case")
    enum class BadUrlTokenParam(
    ...
    @Suppress("unused", "EnumEntryName", "ktlint:standard:enum-entry-name-case")
    enum class NumberOrDimensionTokenParam(
        val value: String,
    ...
    enum class MinifiedCssParams(
        ...
        `no spaces`(
            content =
                ".segment-fill{fill:azure;stroke:#b0c4de;stroke-width:1}" +
        ...
        `no spaces and dimensions starting with dot`(
            content =
                ".cloud{fill:#fff;stroke:#d3d3d3;stroke-width:2;opacity:.8}" +
